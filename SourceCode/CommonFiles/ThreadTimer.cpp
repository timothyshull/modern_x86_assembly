...
bool ThreadTimer::GetProcessAffinityMask(uint64_t* process_mask, uint64_t* system_mask) {
...
bool ThreadTimer::SetProcessAffinityMask(uint64_t process_mask)
{
    BOOL status;
    HANDLE h_process = GetCurrentProcess();
...
bool ThreadTimer::SetThreadAffinityMask(uint64_t thread_mask)
{
    DWORD_PTR status;
    HANDLE h_thread = GetCurrentThread();
...
void ThreadTimer::SaveElapsedTimeMatrix(const char *fn, double *et, int nrows, int ncols)
{
    ...
    if (fopen_s(&fp, fn, "wt") == 0) {
        ...
        sprintf(buff, sizeof(buff), "ThreadTimer::SaveElapsedTimeMatrix - File open error: %s\n", fn);
