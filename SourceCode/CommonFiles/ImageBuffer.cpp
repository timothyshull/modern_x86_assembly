    ...
    if (Type == PixelType::Gray8) {
        if (Width % 4 == 0) {
            ...
            throw new runtime_error(
                    "ImageBuffer::SaveToBitmapFile(const wchar_t* filename) - Bitmap width must be evenly divisible by 4"
            );
        }
        ...
        for (int i = 0; i < Height; i++) {
            for (int j = 0; j < Width; j++) {
                ...
                if (bm->SetPixel(j, i, c) != Gdiplus::Ok) {
                    throw new runtime_error(
                            "ImageBuffer::SaveToBitmapFile(const wchar_t* filename) - SetPixel() error occurred"
                    );
                }
            }
        }
...
void ImageBuffer::AllocRgbArrays(Uint8 * *r, Uint8 * *g, Uint8 * *b, size_t
alignment)
{
...
*
r = (Uint8 * )
_aligned_malloc(Width
* Height * sizeof(Uint8), alignment);
*
g = (Uint8 * )
_aligned_malloc(Width
* Height * sizeof(Uint8), alignment);
...
void ImageBuffer::SetRgbPixelData(const Uint8 *r, const Uint8 *g, const Uint8 *b)
{
    if (Type != PixelType::Rgb32) {
        throw new runtime_error(
                "ImageBuffer::SetRgbPixelData(const Uint8* r, const Uint8* g, const Uint8* b) - Invalid 'Type' value"
        );
    }
